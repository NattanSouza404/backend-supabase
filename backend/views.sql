...
CREATE VIEW vw_itens_vendidos WITH (security_invoker = on) AS
	SELECT
...
-- Uma view geral sobre os pedidos (se usa ela na edge function de enviar e-mail)
CREATE VIEW vw_pedidos WITH (security_invoker = on) AS  
  SELECT
		ped_id AS id_pedido,
		ped_created_at AS data_pedido,
